...
function average = DBA_one_iteration(averageS,sequences)
	...
	tupleAssociation = cell (1, size(averageS,2));% timestamps 
	...
	for k=1:length(sequences)
		...
		for j=2:size(sequence,2)
		    ...
		    if indiceRes==0
			res = costMatrix(i-1,j-1);%match
		    elseif indiceRes==1
			res = costMatrix(i,j-1);%insertion
		    elseif indiceRes==2
			res = costMatrix(i-1,j);%deletion
		    end
		...
		if pathMatrix(i,j)==0%match
		    i=i-1;
		    j=j-1;
		elseif pathMatrix(i,j)==1%insertion
		    j=j-1;
		elseif pathMatrix(i,j)==2%deletion
		    i=i-1;          
		else
		    break;
		end
